...
class PrefectureImagePage extends StatelessWidget {
  PrefectureImagePage({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('旧レジェンドステージ'),
        centerTitle: true,
      ),
      body: PageView.builder(
        itemCount: legend.length, 
        itemBuilder: (context, index) {
          final prefectureName = legend[index];
          return _buildPrefecturePage(prefectureName); 
        }
      ),
    );
  }

  Widget _buildPrefecturePage(String prefectureName) {
    if (prefectureImages.containsKey(prefectureName)) {
      final images = prefectureImages[prefectureName]!;
      final imageWidgets = images.entries.map((entry) {
        final stageName = entry.key;
        final imageNames = entry.value;
        return Column(
      ...
      // prefectureNameに対する画像がない場合の処理
      return Center(
        child: Text(
          'No images found for $prefectureName',
